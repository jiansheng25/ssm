...
public interface ISchoolDao {
    ...
    List<Student> findAllStudent(@Param("startIndex") Integer startIndex);
    ...
    List<Student> findStudentByXyId(@Param("xyId") Integer xyId,@Param("startIndex") Integer startIndex);

    //根据学院获取班级
    @Select("select * from class where xyId=#{xyId} order by stuClass asc")
    List<Class> findClassByXyId(@Param("xyId") Integer xyId);

    //根据班级获取学生
    @Select("select * from student as s left join class as c on s.stuClass=c.stuClass" +
            " left join xueyuan as x on c.xyId=x.xyId where s.stuClass=#{stuClass} limit #{startIndex},14")
    List<Student> findStudentByClass(@Param("startIndex") Integer startIndex,@Param("stuClass") String stuClass);
